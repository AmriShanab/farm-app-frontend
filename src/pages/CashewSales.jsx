...
export default function CashewSales() {
  ...
  useEffect(() => {
    ...
    const loadSales = async () => {
      ...
      try {
        ...
        if (isActive)
          setError(
            "Failed to load cashew sales data. Please check your connection.",
          );
      } finally {
  ...
  const avgRate = filtered.length
    ? (
        filtered.reduce((a, s) => a + Number(s.rate || 0), 0) / filtered.length
      ).toFixed(0)
    : 0;
  ...
  const handleUpdateSale = async () => {
    ...
    try {
      ...
      setSales((prev) =>
        prev.map((s) => (s.id === editSale.id ? completeRecord : s)),
      );
      closeEditSale();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          ...
          return (
            <div
              key={i}
              className="relative overflow-hidden rounded-[1.25rem] p-4 bg-gradient-to-br from-[#9a3412] to-[#7c2d12] text-white shadow-lg shadow-orange-900/20 group border border-orange-800/50 transition-all hover:shadow-orange-900/40 hover:-translate-y-1 h-28"
            >
        ...
        <div className="bg-gradient-to-b from-orange-50 to-white border border-orange-200 rounded-xl p-6 shadow-md mb-6 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex justify-between items-center mb-5 border-b border-orange-100 pb-3">
            <h3 className="text-lg font-black text-orange-900 flex items-center gap-2">
              <Nut size={18} className="text-orange-600" /> Record Cashew
              Harvest Yield
            </h3>
            <button
              onClick={cancelAdd}
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-bold focus:border-orange-500 focus:ring-2 focus:ring-orange-100"
                disabled={isSaving}
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                value={newRow.farm}
                onChange={handleRowChange}
                className="w-full p-2.5 text-sm bg-white border border-gray-300 rounded-lg outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 font-bold"
                disabled={isSaving}
              >
            ...
            <div>
              <label className="block text-[11px] font-black text-red-600 uppercase tracking-wider mb-1">
                Labor Cost Ded.
              </label>
              <input
                type="number"
                name="laborCost"
                placeholder="0.00"
                value={newRow.laborCost}
                onChange={handleRowChange}
                className="w-full p-2.5 text-sm border border-red-200 bg-red-50/50 rounded-lg outline-none focus:border-red-500 font-bold text-red-700"
                disabled={isSaving}
              />
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-100">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isSaving}
                className="px-6 py-2.5 bg-orange-600 rounded-lg text-white text-sm font-bold shadow-md hover:bg-orange-700 flex items-center gap-2 transition-all disabled:opacity-50"
              >
                {isSaving ? (
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              {!isLoading &&
                filtered.map((sale) => {
                  ...
                  return (
                    <tr
                      key={sale.id}
                      className={`border-t border-gray-50 transition-colors ${isSel ? "bg-orange-50/40" : "hover:bg-gray-50/40"}`}
                    >
                      <td className="p-4">
                        <input
                          type="checkbox"
                          ...
                          className="rounded border-gray-300 text-orange-600 focus:ring-orange-500 accent-orange-600 cursor-pointer"
                        />
                      </td>
                      ...
                      <td className="p-4">
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
                          {sale.farm === "MR2" ? "MR2" : "MR1"}
                        </span>
                      </td>

                      <td className="p-4">
                        <span className="font-bold text-gray-900">
                          {Number(sale.kg || 0).toLocaleString()}
                        </span>
                      ...
                      <td className="p-4">
                        {Number((sale.laborCost ?? sale.labor_cost) || 0) >
                        0 ? (
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <button className="px-3 py-1 text-xs font-black rounded bg-gradient-to-br from-orange-600 to-orange-700 text-white">
              1
            </button>
            <button className="p-1.5 rounded border border-gray-300 bg-white text-gray-500 hover:bg-gray-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[1.5rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col animate-in fade-in zoom-in-95">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-black text-gray-900">
                    Update Cashew Record
                  </h2>
                  <p className="text-xs text-gray-400 font-medium">
                    Modifying entry #{editSale.id}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-gray-900 font-bold focus:border-orange-500 focus:outline-none transition-all"
                  />
                </div>
                <div>
                  <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">
                  ...
                  <select
                    ...
                    value={editRow.farm}
                    onChange={handleEditRowChange}
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-gray-900 font-bold focus:border-orange-500 focus:outline-none transition-all cursor-pointer"
                  >
                    <option value="MR1">MR1 Farm</option>
                    <option value="MR2">MR2 Farm</option>
                  </select>
              ...
              <div>
                <label className="block text-xs font-black text-red-600 uppercase tracking-wider mb-1.5">
                  Labor Cost Deduction (-)
                </label>
                <input
                  type="number"
                  name="laborCost"
                  value={editRow.laborCost}
                  onChange={handleEditRowChange}
                  className="w-full border border-red-200 bg-red-50 text-red-700 rounded-xl px-4 py-2.5 font-bold focus:border-red-500 focus:outline-none transition-all"
                />
              </div>
            </div>
