...
const calculateWarranty = (purchaseDate, months) => {
  ...
  const expiry = new Date(
    purchase.setMonth(purchase.getMonth() + parseInt(months)),
  );
  const now = new Date();
...
export default function AssetManagement() {
  ...
  const startEdit = (asset) => {
    ...
    setNewRow({
      ...
      warrantyMonths: String(
        asset.warrantyMonths ?? asset.warranty_months ?? "",
      ),
    ...
    try {
      ...
      if (editingId) {
        ...
        setAssets((prev) =>
          prev.map((a) =>
            a.id === editingId
              ? { ...a, ...updated, ...payload, id: editingId }
              : a,
          ),
        );
      } else {
  ...
  const totalValue = assets.reduce(
    (acc, curr) =>
      acc + parseFloat(curr.purchaseCost || curr.purchase_cost || 0),
    0,
  );
  const activeWarranties = assets.filter(
    (a) =>
      calculateWarranty(
        a.purchaseDate || a.purchase_date,
        a.warrantyMonths || a.warranty_months,
      ).active,
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-gray-500 pl-[52px]">
            Track high-value equipment, supplier details, and warranty
            lifecycles
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              return (
                <div
                  key={i}
                  className="relative overflow-hidden rounded-[1.25rem] p-4 bg-gradient-to-br from-green-700 to-green-800 text-white shadow-lg shadow-green-900/20 group border border-green-800/40 transition-all hover:shadow-green-900/40 hover:-translate-y-1 h-28"
                >
                  ...
                  <h3 className="text-2xl font-bold relative z-10 mb-3 tracking-tight truncate">
                    {card.amount}
                  </h3>
        ...
        <div className="md:col-span-4 flex items-start">
          <div className="w-full bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col justify-center gap-3 h-full">
            ...
            >
              <option value="All">All Estates (MR1, MR2 & Floating)</option>
        ...
        <div className="bg-gradient-to-b from-green-50 to-white border border-green-200 rounded-xl p-6 shadow-md mb-6 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
            ...
            <div className="md:col-span-6">
              ...
              <input
                type="text"
                placeholder="e.g. TAFE 45DI Tractor"
                value={newRow.name}
            ...
            <div className="md:col-span-3">
              ...
              <select
                value={newRow.farm}
                onChange={(e) => setNewRow({ ...newRow, farm: e.target.value })}
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-bold bg-white focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                disabled={isSaving}
              >
            ...
            <div className="md:col-span-3">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewRow({ ...newRow, purchaseDate: e.target.value })
                }
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-bold focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                disabled={isSaving}
              />
            ...
            <div className="md:col-span-6">
              ...
              <input
                type="text"
                placeholder="e.g. TAFE Motors Lanka"
                value={newRow.supplier}
                onChange={(e) =>
                  setNewRow({ ...newRow, supplier: e.target.value })
                }
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                disabled={isSaving}
              />
            </div>
            <div className="md:col-span-3">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewRow({ ...newRow, purchaseCost: e.target.value })
                }
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-black text-gray-900 focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                disabled={isSaving}
              />
            </div>
            <div className="md:col-span-3">
              ...
              <input
                type="number"
                placeholder="e.g. 24"
                value={newRow.warrantyMonths}
                onChange={(e) =>
                  setNewRow({ ...newRow, warrantyMonths: e.target.value })
                }
                className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-bold focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                disabled={isSaving}
              />
            ...
            <div className="md:col-span-12">
              ...
              <input
                type="text"
                placeholder="Details about serial numbers, specific conditions, etc."
                value={newRow.notes}
                onChange={(e) =>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="px-6 py-2.5 bg-green-600 rounded-lg text-white text-sm font-bold shadow-md hover:bg-green-700 flex items-center gap-2 transition-colors disabled:opacity-70"
            >
              {isSaving ? (
                  ...
                  assets.map((asset) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-1 rounded text-[10px] font-bold tracking-wide flex items-center w-max gap-1 ${
                              asset.farm === "MR1"
                        ...
                        <td className="p-4">
                          <div className="flex flex-col items-center">
                            <span
                              className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider flex items-center gap-1 ${
                                warranty.active
                                  ? "bg-green-100 text-green-700"
