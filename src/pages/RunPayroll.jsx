...
export default function RunPayroll() {
  ...
  const exportSlipPdf = (emp) => {
    ...
    win.document.write(`<!doctype html><html><head><meta charset="utf-8" />
      ...
      <div class="grid">
        <div class="card">
          <div class="card-hd">Salary & Wage Composition</div>
          <div class="card-bd tri">
            ...
            <div>
              ...
              ${(emp.basicRate || 0) > 0 ? `<div class="sub">Bas. Rate: ${money(emp.basicRate)}</div>` : ""}
            </div>
            <div>
  ...
  const handleViewSettledSlip = async (emp) => {
    ...
    if (matchingRun) {
      try {
        ...
        const payout = details.payouts?.find(
          (p) => String(p.empId) === empIdStr,
        );
        if (payout) {
          setBreakdownEmp({
            ...payout,
    ...
    >
      ...
      {payFrequency === "batch" ? (
        ...
        <>
          ...
          >
                        ...
                        <td style={{ ...tdStyle(), textAlign: "right" }}>
                          ...
                          {emp.grossPay > 0 && (
                            <div className="text-[10px] font-semibold text-gray-400 mt-0.5">
                              B: {fmt(emp.basicPay)} · A:{" "}
                              {fmt(emp.allowancePay)}
                            </div>
                          )}
                        </td>
                ...
                {filtered.length > 0 && (
                  <tfoot>
                    ...
                    >
                      ...
                      >
                        {filtered.reduce((s, e) => s + (e.fullDays || 0), 0)}
                      </td>
                      <td
                        style={{
                          ...tdStyle(),
                      ...
                      >
                        {filtered.reduce((s, e) => s + (e.halfDays || 0), 0)}
                      </td>
                      <td style={{ ...tdStyle(), textAlign: "right" }}></td>
                      <td
                        style={{
                      ...
                      >
                        Rs.{" "}
                        {fmt(
                          filtered.reduce((s, e) => s + (e.grossPay || 0), 0),
                        )}
                      </td>
                      <td
                        style={{
            ...
            >
              <span
                style={{ fontSize: "12px", color: "#6b7a6b", fontWeight: 600 }}
              >
                Showing{" "}
              ...
              {historyRows.length > 0 && (
                <tfoot>
                  <tr className="border-t-2 border-gray-200 bg-gray-50/80">
                    <td className="py-2.5 px-4 text-[10px] font-black text-gray-700 uppercase tracking-wider">
                      Totals
                    </td>
                    <td className="py-2.5 px-4"></td>
                    <td className="py-2.5 px-4 text-right text-xs font-black text-gray-900">
                      Rs.{" "}
                      {fmt(
                        historyRows.reduce(
                          (s, r) => s + (parseFloat(r.totalGross) || 0),
                          0,
                        ),
                      )}
                    </td>
                    <td className="py-2.5 px-4 text-right text-xs font-black text-orange-700">
                      Rs.{" "}
                      {fmt(
                        historyRows.reduce(
                          (s, r) => s + (parseFloat(r.totalDeductions) || 0),
                          0,
                        ),
                      )}
                    </td>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              {managerSalaries.length > 0 && (
                <tfoot>
                  <tr className="border-t-2 border-gray-200 bg-gray-50/80">
                    <td
                      className="py-2.5 px-4 text-[10px] font-black text-gray-700 uppercase tracking-wider"
                      colSpan={2}
                    >
                      Totals
                    </td>
                ...
                <div className="p-5 grid gap-4 text-sm bg-white">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                    ...
                    {details.length > 0 && (
                      <div className="px-4 py-2 bg-gray-50/60 border-t border-gray-100 flex justify-between text-xs font-black">
                        ...
                        <span
                          className={
                            overGross ? "text-red-600" : "text-orange-700"
                          }
                        >
                          Rs. {fmt(deduct)}
                        </span>
                  ...
                  {overGross && (
                    <p className="text-[11px] font-bold text-red-600">
                      Recovery exceeds gross pay. Lower an amount or uncheck an
                      advance.
                    </p>
                  )}
      ...
      {breakdownEmp && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          />
          <div className="relative z-10 w-full max-w-3xl rounded-2xl bg-white shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm bg-white overflow-y-auto flex-1 min-h-0 items-start content-start">
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
                ...
                <div className="p-4 grid grid-cols-3 divide-x divide-gray-100 text-center">
                  <div>
                    <span className="block text-lg font-black text-gray-700">
                  ...
                  <div>
                    ...
                    {(breakdownEmp.basicRate || 0) > 0 && (
                      <span className="block text-[10px] font-bold text-gray-500 mt-0.5">
                        Bas. Rate: Rs. {fmt(breakdownEmp.basicRate)}
                      </span>
                    )}
                  </div>
                  <div>
                ...
                breakdownEmp.advanceDetails.length === 0 ? (
                  ...
                  <ul className="divide-y divide-gray-50">
                    {breakdownEmp.advanceDetails.map((adv) => {
                      ...
                      const isPartial =
                        original > 0 && deducted + 0.01 < original;
                      return (
                        <li
                          key={adv.id}
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          onClick={() =>
                            setBreakdownEmp({
                              ...emp,
                              periodStart: historicalRun.startDate,
                              periodEnd: historicalRun.endDate,
                              paidOn: historicalRun.finalizedAt,
                            })
                          }
