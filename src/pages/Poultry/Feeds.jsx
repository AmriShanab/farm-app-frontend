...
export default function PoultryFeeds() {
  ...
  const handleSave = async () => {
    if (!newRow.feedType || !newRow.quantity || !newRow.ratePerUnit) {
  ...
  const startEditing = (log) => {
    ...
    setEditRow({
      ...
      ratePerUnit: log.rate_per_unit || "",
      paidAmount: log.paid_amount || "",
      chequeNo: log.cheque_no || "",
      chequeDate: log.cheque_date || "",
    });
  };

  const handleEditSave = async () => {
    if (!editRow.feedType || !editRow.quantity || !editRow.ratePerUnit) {
      toast.error("Please fill required fields.");
      return;
    }
    setIsSaving(true);
    try {
      ...
      const updatedFeed = await updatePoultryFeed(editingId, payload);
      setData(data.map((item) => (item.id === editingId ? updatedFeed : item)));
      setEditingId(null);
      toast.success("Feed record updated successfully.");
    } catch {
      toast.error("Failed to update feed record.");
    } finally {
      setIsSaving(false);
    }
  };
