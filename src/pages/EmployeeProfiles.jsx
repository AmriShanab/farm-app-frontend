...
const splitSalary = (wage, type, basicRate) => {
  ...
  return {
    basic,
    allowance: Math.max(0, total - basic),
    unit: type === "fixed" ? "mo" : "day",
  };
};
...
export default function EmployeeProfiles() {
    ...
    const load = async () => {
      ...
      try {
        const all = await getEmployees(
          farmFilter === "All" ? null : farmFilter,
          null,
        );
        ...
        if (isActive)
          setError(
            "Failed to load employee directory. Please check your connection.",
          );
      } finally {
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      ...
      setEmployees((prev) =>
        prev.map((e) => (e.id === editEmployee.id ? record : e)),
      );
      closeEditEmployee();
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm font-medium text-gray-500 pl-[52px]">
              Maintain estate workforce details, base locations, and salary
              structures
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {[
            ...
            return (
              <div
                key={i}
                className="relative overflow-hidden rounded-[1.25rem] p-4 bg-gradient-to-br from-[#166534] to-[#14532d] text-white shadow-lg shadow-green-900/20 group border border-green-800/50 transition-all hover:shadow-green-900/40 hover:-translate-y-1 h-28"
              >
          ...
          <div className="bg-gradient-to-b from-green-50 to-white border border-green-200 rounded-xl p-6 shadow-md mb-6 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="flex justify-between items-center mb-5 border-b border-green-100 pb-3">
              <h3 className="text-lg font-black text-green-900 flex items-center gap-2">
                <UserPlus size={18} className="text-green-600" /> Onboard New
                Employee
              </h3>
              <button
                onClick={cancelAdd}
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
              <div className="md:col-span-4">
                ...
                <input
                  ...
                  className="w-full p-2.5 text-sm border border-gray-300 rounded-lg outline-none font-bold focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-all"
                  disabled={isSaving}
                />
              </div>
              ...
              <div className="md:col-span-4">
                ...
                <input
                  ...
                  value={newRow.role}
                  onChange={handleRowChange}
                  className="w-full p-2.5 text-sm bg-white border border-gray-300 rounded-lg outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 font-bold"
                  disabled={isSaving}
                />
              </div>
              ...
              <div className="md:col-span-4">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[9px] font-bold text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-100">
                    Required
                  </span>
                </div>
                <select
            ...
            <div className="flex justify-end items-center mt-6 pt-4 border-t border-gray-100 gap-3">
              ...
              <button
                ...
                disabled={isSaving}
                className="px-6 py-2.5 bg-green-600 rounded-lg text-white text-sm font-bold shadow-md hover:bg-green-700 flex items-center gap-2 transition-all disabled:opacity-50"
              >
                {isSaving ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                {!isLoading &&
                  filtered.map((emp) => {
                    ...
                    return (
                      <tr
                        key={emp.id}
                        className={`border-t border-gray-50 transition-colors ${isSel ? "bg-green-50/40" : "hover:bg-gray-50/40"}`}
                      >
                        <td className="p-4">
                          <input
                            type="checkbox"
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border ${emp.status === "paused" ? "bg-amber-100 text-amber-700 border-amber-200" : "bg-green-100 text-green-700 border-green-200"}`}
                            >
                              {emp.name.substring(0, 2).toUpperCase()}
                            </div>
                            <div className="flex items-center gap-2">
                              <span
                                className={`font-bold text-[13px] ${emp.status === "paused" ? "text-gray-400" : "text-gray-900"}`}
                              >
                                {emp.name}
                              </span>
                              {emp.status === "paused" && (
                                <span className="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-100 text-amber-700 border border-amber-200">
                                  Paused
                                </span>
                              )}
                            </div>
                        ...
                        <td className="p-4">
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${emp.farm === "Poultry" ? "bg-amber-50 text-amber-700 border border-amber-200" : emp.farm === "Floating" ? "bg-purple-50 text-purple-700 border border-purple-200" : "bg-green-50 text-green-700 border border-green-200"}`}
                          >
                        ...
                        <td className="p-4">
                          <div className="flex gap-1.5">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${payFreqLabel === "Monthly" ? "bg-blue-50 text-blue-700 border border-blue-200" : "bg-orange-50 text-orange-700 border border-orange-200"}`}
                            >
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-col items-end">
                                <span className="text-gray-900 font-black text-[13px]">
                                  Rs. {fmt(parseFloat(wageValue))}
                                </span>
                                <span className="text-[10px] text-gray-400 font-bold uppercase">
                                  /{" "}
                                  {wageTypeLabel === "Daily" ? "Day" : "Month"}
                                </span>
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              ...
              <button className="px-3 py-1 text-xs font-black rounded bg-gradient-to-br from-green-600 to-green-700 text-white">
                1
              </button>
              <button className="p-1.5 rounded border border-gray-300 bg-white text-gray-500 hover:bg-gray-50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-medium">
                      Modifying profile #{editEmployee.id}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-gray-900 font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:outline-none transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">
              ...
              <div className="px-6 py-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black text-gray-900">
                      Rs. {fmt(parseFloat(editRow.wage) || 0)}
                    </p>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className="bg-blue-600 px-6 py-2.5 text-sm shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5 rounded-xl text-white font-bold disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
                  >
                    {isSaving ? (
                      <Loader2 size={16} className="animate-spin" />
                    ) : (
              ...
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Current: Rs. {fmt(basicRate)} / day. A new rate applies
                    going forward — past payslips stay frozen.
                  </p>
                </div>
                <button
                  onClick={() => setIsRateModalOpen(false)}
